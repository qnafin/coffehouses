// Компонент https://github.com/ttdung11t2/react-native-confirmation-code-input
// Взят чтобы внести некоторые правки, не дожидаясь пул реквеста
...
export default class ConfirmationCodeInput extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.value !== value) {
      const codeArrFromProps = new Array(this.props.codeLength).fill('');
      if (value) {
        value.toString().split('').forEach((char, charIndex) => {
          codeArrFromProps[charIndex] = char;
        });
      }

      if (!_.isEqual(codeArr, codeArrFromProps)) {
        this.setState({ codeArr: codeArrFromProps });
      }
    }
  }
  ...
  render() {
    ...
    return (
      <View style={[styles.container, this._getContainerStyle(sizeHeight || size, inputPosition), containerStyle]}>
        {codeInputs}
      </View>
    );
  }
